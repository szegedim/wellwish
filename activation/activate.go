...
// The activation module does not allow any production modules to run until the deployment gets activated.
// This helps to ensure that we can always get into a consistent state at startup.
...
// The activation key can be entered from the user interface at startup, which redirects to the management UI.
// Bookmark the management url with the randomly generated apikey to manage the site.
// There is no other way to access logs, traces, etc.
// If the activation key is empty, no activation is needed.
// Containers start right away. However, you cannot access any management features either without checking the logs.

// TODO Management apikey rotation can cleanly be done in the load balancer replacing the keys
// TODO with a one to one app
